...
**G1：方法可行性开发中，尚未通过。** 已实现从DA3估计出发的相机联合校正。在一个已知双深度解析场景中，固定镜头/共享焦距两阶段方法把保留点三维中位误差约497mm降到29mm，注入47条错配后结果接近。当前4组杆图仍缺可靠对应，未应用新相机；像素检查也存在三维仍差却通过的反例。下一步验证新杆数据与假设失配，再完成共同读取器和独立对象验收；9月27日为冲刺检查点，不能承诺通过。
...
| 第三版读取器、396项配对控制；4份新估计相机预测、18个身份补丁、72项特权相机诊断 | 杆场景有效校正、局部噪声/部分表面读取、跨对象重复收益 |
| 固定镜头下RGB轨迹联合校正原型、两轮18项优化及错配压力对照 | 新杆数据验证、假设失配检测、可信自动应用 |
| Blender配置侧栏、骨架检查、插件打包和CI | 完整导入、增强、比较与撤回流程 |

最新[相机联合优化与固定镜头](docs/experiments/2026-09-23-camera-bundle-and-fixed-lens.md)：两轮18项优化，6组证据不足保持原样。共享焦距两阶段方法在已知解析正例上得到保留像素与三维的同步改善；错误匹配压力下比单次鲁棒拟合更稳定。没有GT初始化、没有新的杆收益、没有重标旧点云；285项测试通过。见[今日日志](docs/journal/2026-09-23.md)、[ADR0013](docs/decisions/0013-shared-calibration-and-training-outlier-control.md)。

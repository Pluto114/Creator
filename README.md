...
**G1：方法可行性开发中，尚未通过。** 相机校正已从解析场景推进到20张新Blender杆图。显式增加参考纹理后，实际DA3初始化下完整杆曲线达到25mm容差内R/P100%、p95约8–10mm；断杆和细杆仍不合格，变焦反例被扣留。评价侧只换真实位姿即可消除固定镜头条件的主要杆误差，下一步优先验证拍摄高度变化与残余位姿精度，再接一致的新基础、共同读取器和独立对象。旧砖墙输入/结果没改，没有发布自动补丁；9月27日仍是冲刺检查点。
...
| 20张新参考纹理杆图、16相机/32曲线条件、24条特权诊断；完整杆取得曲线收益 | 断杆/细杆的残余位姿精度、新几何版本与独立对象验证 |
| Blender配置侧栏、骨架检查、插件打包和CI | 完整导入、增强、比较与撤回流程 |

最新[参考纹理辅助的新杆实验](docs/experiments/2026-09-23-reference-assisted-rods.md)：完整杆两阶段校正后R/P100%，断杆82%/84%、细杆68%/68%，尚未全面达标。断杆保留两段却偏移并侵入真实缺口；只换真实位姿的评价诊断恢复正确。变焦三项校正均扣留。289项测试、20,600条独立射线检查通过；参考板是新增采集条件，三种杆布局不是独立对象。见[今日日志](docs/journal/2026-09-23.md)、[ADR0014](docs/decisions/0014-residual-pose-before-rod-extent-changes.md)。

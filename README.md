...
**G1：方法可行性开发中，尚未通过。** 已完成同20张图的两种DA3估计相机对照：新ray pose仍无正确杆，普通错误接受从2变3。RGB完整轨迹审计显示当前砖墙对应不足；固定空间隔离的前置检查已在独特双深度正例放行、单平面/重复纹理控制拒绝。下一步验证无需GT的鲁棒相机校正，当前尚未实现。共同读取器与独立对象验收仍欠账；9月27日为冲刺检查点，不能承诺通过。

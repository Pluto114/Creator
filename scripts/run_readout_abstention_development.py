"""Replay fixed closed/abstaining readers on 234 development conditions."""
...
from creator_eval.common_readout_abstention import DEFAULTS as VALLEY_DEFAULTS  # noqa: E402
from creator_eval.common_readout_abstention import readout as valley_readout  # noqa: E402
from creator_eval.common_readout_valley_closed import DEFAULTS as SECTION_DEFAULTS  # noqa: E402
from creator_eval.common_readout_valley_closed import readout as section_readout  # noqa: E402
from creator_eval.line_controls import curve_metrics, gap_coverage  # noqa: E402

CONFIG = ROOT / "configs/readout_abstention_development_v1.json"
RUN_ID = "readout-abstention-development-v1-20260924"
READERS = {"persistent_valley_closed": section_readout, "abstaining_components": valley_readout}
POLICIES = {"persistent_valley_closed": SECTION_DEFAULTS, "abstaining_components": VALLEY_DEFAULTS}
SOURCES = ["scripts/run_readout_abstention_development.py", "experiments/src/creator_eval/readout_background_controls.py",
           "tests/test_common_readout_abstention.py", "experiments/src/creator_eval/common_readout.py",
           ...
           "experiments/src/creator_eval/common_readout_valley_closed.py", "experiments/src/creator_eval/common_readout_abstention.py"]
NEW_FILES = [SOURCES[0], SOURCES[2], "experiments/src/creator_eval/common_readout_abstention.py", "configs/readout_abstention_development_v1.json"]
...
def prepare(config_path):
    ...
    if config["readers"] != list(READERS) or config["readout"] != {}:
        raise ValueError("Use frozen closed/abstaining defaults without overrides")
...
def evaluate(run_id, output):
    ...
    for record in inference["records"]:
        ...
        rows.append(dict(case_id=record["case_id"], reader=record["reader"], **labels, state=result["state"], segments=result["segments"],
            ...
            valley_checks=result.get("split_valley_checks"), resolution_state=result.get("resolution_state"),
            unresolved_component_count=result.get("unresolved_component_count", 0),
            unresolved_components=result.get("unresolved_components", [])))
    ...
    previous = read(ROOT / config["baseline_results_file"])
    prior = {(row["source_dataset"], row["original_case_id"]): row
             for row in previous["rows"] if row["reader"] == "persistent_valley_closed"}
    baseline = [row for row in rows if row["reader"] == "persistent_valley_closed"]
    assert len(baseline) == 234
    for row in baseline:
        old = prior[(row["source_dataset"], row["original_case_id"])]
        for key in ("state", "segments", "metrics", "gap", "diagnostic_qualified", "split_decisions", "valley_checks"):
            assert row[key] == old[key], (row["original_case_id"], key)
    baseline_parity = dict(identical_closed_rows=len(baseline),
                          source_results_sha256=digest(ROOT / config["baseline_results_file"]))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "docs/experiments/results/2026-09-24-readout-abstention-development.json")

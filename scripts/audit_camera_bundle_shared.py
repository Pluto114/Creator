"""Audit the fixed-lens follow-up without changing its first pilot records."""
...
def run():
    ...
    for receipt in inference["records"]:
        ...
        if "extrinsics" in result:
            ...
            reference_k = shared_square_intrinsics(k) if record["variant"]["common_intrinsics"] else k
            assert np.allclose(corrected_k[:, [0, 1], [0, 1]] / reference_k[:, [0, 1], [0, 1]], result["focal_scale"])
    ...
    assert len(rows) == 10 and len(inference["skipped"]) == 6
    ...
    for axis, key, label in ((axes[0], "median_px", "Held-out median error (px)"), (axes[1], "p95_px", "Held-out p95 error (px)")):
        ...
        axis.set(ylabel=label, xticks=x, xticklabels=names, yscale="log")
    ...
    axes[2].set(ylabel="Held-out point median error (mm), GT evaluation", xticks=x, xticklabels=names, yscale="log")
    axes[2].tick_params(axis="x", labelrotation=65, labelsize=8)
    axes[2].legend(fontsize=8)
    fig.suptitle("Fixed-lens bundle follow-up | same known analytic layout | no rod efficacy claim", fontsize=13)
    figure = ROOT / "docs/experiments/figures/2026-09-23-camera-bundle-shared.png"
    ...
    write_json(ROOT / "docs/experiments/results/2026-09-23-camera-bundle-shared-audit.json", report)
    print("CAMERA_SHARED_BUNDLE_AUDIT", len(rows), "condition gauges and holdout contracts", flush=True)

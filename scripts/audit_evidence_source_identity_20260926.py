"""Audit prior frozen sources plus the Sept 26 paired-evidence continuation.
...
OUTPUT = ROOT / 'docs/experiments/results/2026-09-26-evidence-git-freeze-audit.json'
...
def audit(output):
    ...
    for run_id, metadata_name in RUNS:
        ...
        if run_id.endswith('20260926') and metadata.get('run_id', metadata.get('audit_run_id')) != run_id:
            raise ValueError('Metadata run identity mismatch: ' + run_id)
        if run_id.endswith('20260926') and 'source_freeze_sha256' in metadata:
        ...
        if run_id == 'rod-validation-evidence-audit-v1-20260926':
    ...
    result = dict(state='complete', audited_git_head=head, active_frozen_run_count=len(runs),
        historical_frozen_run_count=12, added_frozen_run_count=4,
        ...
        reproduction='Load scripts/Enter-CreatorEnvironment.ps1, then backends/da3/.venv/Scripts/python.exe -B scripts/audit_evidence_source_identity_20260926.py --output <new-output.json>',
        ...
        scope='Raw live/snapshot/freeze equality for sixteen active runs plus Git byte policy and all explicit continuation reporting/checking sources. Ongoing run completion, scores, and documentation links are outside this source-only audit.')
    with output.open('x', encoding='utf-8', newline='\n') as stream:
        json.dump(result, stream, indent=2, ensure_ascii=False, allow_nan=False)
        stream.write('\n')
    print('EVIDENCE_SOURCE_IDENTITY', len(runs), 'runs;', reference_count, 'active source references;',

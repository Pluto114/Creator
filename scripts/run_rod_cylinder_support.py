"""Freeze and evaluate one-pass view exclusion on the same development RGB pools."""
...
from creator_eval.rod_cylinder_support import (  # noqa: E402
    select_supported_cylinders as select_cylinder_hypothesis,
)
...
CONFIG = ROOT / "configs/rod_cylinder_support_v1.json"
RUN_ID = "rod-cylinder-support-v1-20260920"
SHARE = ROOT / "docs/experiments/results/2026-09-20-cylinder-support.json"
SOURCES = [*BASE_SOURCES, "experiments/src/creator_eval/rod_radius_consistency.py",
           ...
           "experiments/src/creator_eval/rod_cylinder_support.py", "scripts/run_rod_cylinder_support_controls.py",
           "scripts/run_rod_cylinder_support.py"]
...
def prepare(config_path):
    ...
    print("PREPARED_CYLINDER_SUPPORT", len(records), "associations", len(expected_keys(method)), "conditions", flush=True)
...
def evaluate(config_path, share):
    ...
    for source_run, listing in ((run, inference), (baseline, read_json(baseline / "inference.json"))):
        ...
        for entry in listing["associations"]:
            ...
            if source_run == run:
                ...
                audit["support_selection"] = record["association"]["cylinder_support"]
                audits.append(audit)
    ...
    print("EVALUATED_CYLINDER_SUPPORT", len(scored[0]), "paired conditions", flush=True)

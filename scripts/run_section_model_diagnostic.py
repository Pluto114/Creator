"""Freeze and run a 64-condition conditional cross-section identifiability diagnostic."""
...
from creator_eval.section_model_diagnostic import DEFAULTS, diagnose  # noqa: E402
...
CONFIG = ROOT / "configs/section_model_diagnostic_v1.json"
SHARE = ROOT / "docs/experiments/results/2026-09-26-section-model-diagnostic.json"
SOURCES = ["scripts/run_section_model_diagnostic.py", "experiments/src/creator_eval/section_model_diagnostic.py",
...
def prepare():
    for folder in (RUN, INPUTS, GT):
        assert not folder.exists(), "Keep old diagnostic runs"
    ...
    write(RUN / "source_freeze.json", dict(source_sha256=hashes, protocol_sha256=digest(RUN / "protocol.json"),
        note="Before final run input artifacts or any fitted benchmark scores are produced"))
    cases, truth = generate(protocol)
    INPUTS.mkdir(parents=True)
    GT.mkdir(parents=True)
    inputs = []
    for case in cases:
        path = INPUTS / (case["case_id"]+".npy")
        with path.open("xb") as f:
            np.save(f, case["points"], allow_pickle=False)
        inputs.append(dict(case_id=case["case_id"], points_file=path.name, points_sha256=digest(path), voxel_size=case["voxel_size"]))
    write(INPUTS / "manifest.json", dict(run_id=RUN_ID, method=DEFAULTS, cases=inputs, truth_excluded=True))
    write(GT / "manifest.json", dict(run_id=RUN_ID, input_sha256=digest(INPUTS / "manifest.json"), cases=truth))
    write(RUN / "prepared.json", dict(run_id=RUN_ID, source_sha256=hashes,
        ...
        new_git_filter_bytes_unchanged=True))
    print("PREPARED_SECTION_DIAGNOSTIC", len(cases), flush=True)
...
def infer():
    ...
    for case in manifest["cases"]:
        ...
        print("SECTION_DIAGNOSTIC", case["case_id"], f"{records[-1]['elapsed_seconds']:.2f}s", flush=True)
    checked()
...
def evaluate():
    ...
    write(SHARE, output)
    print("EVALUATED_SECTION_DIAGNOSTIC", len(rows), "model-fold rows; no model accepted", flush=True)

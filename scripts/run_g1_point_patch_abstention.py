"""Complete historical pair replay with the separately frozen abstaining reader."""
...
CONFIG = ROOT / "configs/g1_point_patch_abstention_v1.json"
RUN_ID = "g1-point-patch-abstention-v1-20260924"
SOURCES = ["scripts/run_g1_point_patch_abstention.py", "experiments/src/creator_eval/common_readout_components.py", "reconstruction/src/creator_recon/domain/point_patch.py", "experiments/src/creator_eval/common_readout.py",
           ...
           "experiments/src/creator_eval/common_readout_sections.py", "experiments/src/creator_eval/common_readout_valley_closed.py", "experiments/src/creator_eval/common_readout_abstention.py", "tests/test_common_readout_abstention.py"]
...
def prepare(path):
    ...
    for name in ("scripts/run_g1_point_patch_abstention.py", "experiments/src/creator_eval/common_readout_abstention.py", "configs/g1_point_patch_abstention_v1.json"):
    ...
    for fraction in method["voxel_camera_span_fractions"]:
        ...
        for label, collection in (("base", base), ("candidate", candidate)):
            ...
            graphs.append(dict(fraction=fraction, variant=label, path=path.relative_to(run).as_posix(), sha256=file_hash(path),
                ...
                resolution_state=output["resolution_state"], unresolved_component_count=output["unresolved_component_count"]))
        ...
        for graph in record["graphs"]:
            ...
            for tolerance in config["evaluation"]["tolerances_m"]:
                ...
                rows.append(dict(job_id=job["candidate_job_id"], camera_mode=job["camera_mode"], fraction=graph["fraction"], variant=graph["variant"],
                    ...
                    resolution_state=output["resolution_state"], unresolved_component_count=output["unresolved_component_count"],
                    unresolved_components=output["unresolved_components"],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--share-json", type=Path, default=ROOT / "docs/experiments/results/2026-09-24-point-patch-abstention.json")

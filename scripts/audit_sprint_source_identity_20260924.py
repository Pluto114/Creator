"""Audit raw frozen source identity and Git normalization for the Sept 24 sprint.
...
OUTPUT = ROOT / 'docs/experiments/results/2026-09-24-sprint-git-freeze-audit.json'
...
def audit(output):
    ...
    result = dict(state='complete', audited_git_head=head, active_frozen_run_count=len(runs),
        ...
        reproduction='Load scripts/Enter-CreatorEnvironment.ps1, then backends/da3/.venv/Scripts/python.exe -B scripts/audit_sprint_source_identity_20260924.py --output <new-output.json>',
        ...
        scope='Raw live/snapshot/freeze equality for eight active runs plus Git byte policy. Ongoing run completion, scores, and documentation links are outside this source-only audit.')
